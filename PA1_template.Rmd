...
The following code is used to read in the data in to R:
```{r}
...
The following code is used to create a date.time column that combines the date and interval columns.
```{r}
...
I create a time colum for the purposes of For analyzing the means at the different times of day.
```{r}
...
The following are the mean and median for the total steps per day:
```{r}
mean(total.steps)
median(total.steps)
```

The following histogram displays the distribution of total number of steps per day:
```{r histogram}
library(ggplot2)
...
Here, we calculate the mean steps for each five minute interval, and then put it in a data frame.
```{r}
